...
function spaces_registerevent(spaceId, eventName) {
    let callback = spaces_eventListeners.find(e => e.spaceId === spaceId && e.eventName === e.eventName);
    if (!callback) {
...
function spaces_notifyevent(spaceId, eventName, eventArgs) {
    ...
    DotNet.invokeMethodAsync('BlazorSpaces', 'spaces_notifyevent', spaceId, eventName, args);
}